...
def normalize_node_for_frontend(node: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize node structure to match frontend expectations"""
    node_type = node["type"]
    ...
    if "name" not in config:
        config["name"] = node["data"].get("label", f"New {node_type.title()}")
    ...
    if node_type == "trigger":
        ...
        if "system_prompt" in config:
            config["system_instructions"] = config.pop("system_prompt")
        if "system_instructions" not in config:
            config["system_instructions"] = ""
        if "temperature" not in config:
            config["temperature"] = 0.7
        if "expected_output_format" not in config:
            config["expected_output_format"] = "text"
    
    elif node_type == "end":
        if "capture_output" not in config:
            config["capture_output"] = True
        if "show_output" not in config:
...
def get_workflow_templates() -> List[Dict[str, Any]]:
    ...
    # Normalize all nodes in all templates
    for template in templates:
        if "definition" in template and "nodes" in template["definition"]:
            template["definition"]["nodes"] = [
                normalize_node_for_frontend(node)
                ...
                },
                {
                    ...
                    "data": {
                        ...
                        "config": {
                            ...
                            "system_instructions": """You are a sentiment analysis expert. Analyze the customer feedback and respond with ONLY a JSON object:
{
